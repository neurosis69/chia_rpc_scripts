...
curl -s --insecure --cert ~/.chia/mainnet/config/ssl/wallet/private_wallet.crt --key ~/.chia/mainnet/config/ssl/wallet/private_wallet.key -d "{\"wallet_id\": \"$1\", \"start\":$ST, \"end\":$ET}" -H "Content-Type: application/json" -X POST https://localhost:9256/get_transactions | jq -r --argjson transtype $2 '.transactions[] | if $transtype == -1 then select(keys != "additions") else select((.type == $transtype) and (keys != "additions")) end | [.amount,.confirmed,.confirmed_at_height,.created_at_time,.fee_amount,.name,.sent,.to_address,.to_puzzle_hash,.trade_id,.type,.wallet_id] | map(. // "null") | @csv'
