...
curl -s --insecure --cert _SSLPATH_/private_wallet.crt --key _SSLPATH_/private_wallet.key -d "{\"transaction_id\":\"${1}\"}" -H "Content-Type: application/json" -X POST https://_IP_:9256/get_transaction| jq -r '.transaction | [.amount,.confirmed,.confirmed_at_height,.created_at_time,.fee_amount,.name,.sent,.to_address,.to_puzzle_hash,.trade_id,.type,.wallet_id] | map(. // "null")| @csv'

echo -e "\nJSON Output:"
curl -s --insecure --cert _SSLPATH_/private_wallet.crt --key _SSLPATH_/private_wallet.key -d "{\"transaction_id\":\"${1}\"}" -H "Content-Type: application/json" -X POST https://_IP_:9256/get_transaction| jq -r '.transaction'
