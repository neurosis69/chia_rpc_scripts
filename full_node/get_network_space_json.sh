...
curl -s --insecure --cert _SSLPATH_/private_full_node.crt --key _SSLPATH_/private_full_node.key -d "{\"newer_block_header_hash\":\"$1\",\"older_block_header_hash\":\"$2\"}" -H "Content-Type: application/json" -X POST https://_IP_:8555/get_network_space | jq